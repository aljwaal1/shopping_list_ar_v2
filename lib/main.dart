...
class _HomeState extends State<Home> {
  ...
  Future<void> load() async {
    ...
    final saved = p.getStringList('items_v4') ?? p.getStringList('items_v3') ?? p.getStringList('items_v2');
  ...
  Future<void> save() async { final p = await SharedPreferences.getInstance(); await p.setStringList('items_v4', items.map((e) => e.encode()).toList()); await p.setBool('reminders', reminders); await p.setInt('reminder_h', reminderTime.hour); await p.setInt('reminder_m', reminderTime.minute); }
  ...
  void addItem() { final title = itemCtrl.text.trim(); if (title.isEmpty) return; setState(() { items.insert(0, Item(title, category, qty, false, DateTime.now())); itemCtrl.clear(); qty = 1; }); save(); SystemSound.play(SystemSoundType.click); }
